...
class InteractiveDirectoryTree {
  ...
  async scanDirectory(node: DirectoryNode): Promise<void> {
    try {
      ...
      for (const entry of sorted) {
        ...
        const childNode = new DirectoryNode(
          entry.path,
          entry.name,
          entry.isDirectory
        );
  ...
  updateVisibleNodes(): void {
    ...
    const currentNodePath =
      ...
      this.cursorPosition < this.visibleNodes.length
    ...
    if (currentNodePath) {
      const newIndex = this.visibleNodes.findIndex(
        (node) => node.path === currentNodePath
      );
  ...
  render(): void {
    ...
    console.log(
      '\x1b[33mNavigation: ↑/↓ Move, Space: Select, Enter: Expand/Collapse\x1b[0m'
    );
    console.log(
      '\x1b[33mActions: S: Save Selected Paths, D: Display Content on Screen, F: Save Content to File, Q: Quit\x1b[0m\n'
    );
    ...
    let startIdx = Math.max(
      0,
      this.cursorPosition - Math.floor(terminalHeight / 2)
    );
    const endIdx = Math.min(
      startIdx + terminalHeight,
    ...
    if (this.debug) {
      ...
      console.log(
        `DEBUG INFO: Active node: ${
          this.visibleNodes[this.cursorPosition]?.path
        }`
      );
      console.log(
        `DEBUG INFO: Total visible nodes: ${this.visibleNodes.length}`
      );
      console.log(
        `DEBUG INFO: Visible range: ${startIdx}-${endIdx - 1} (${
          endIdx - startIdx
        } entries)`
      );
    }
  ...
  renderVisibleNode(node: DirectoryNode, index: number): void {
    ...
    // Render the node
    console.log(
      `${indicator}${prefix}${branchSymbol}${checkSymbol} ${statusSymbol} ${dirColor}${node.name}${reset}`
    );
  }
  ...
  toggleExpand(): void {
    ...
    if (
      this.ignoreDirectories.includes(currentNode.name) &&
  ...
  async loadLargeDirectoryContents(node: DirectoryNode): Promise<void> {
    console.clear();
    console.log(
      `\x1b[33mLoading contents of large directory: ${node.path}\x1b[0m`
    );
  ...
  async viewSelectedContent(outputToFile = false): Promise<void> {
    this.inContentViewMode = true;
    const selectedFiles = this.getSelectedFiles();

    if (selectedFiles.length === 0) {
      console.clear();
      console.log(
        '\x1b[33mNo files selected. Use Space to select files.\x1b[0m'
      );
    ...
    if (!outputToFile && textFiles.length > 0) {
        ...
        try {
          ...
          while (!exitViewer) {
            ...
            if (key === 'q' || key === '\u001B') {
              ...
              // Invalid or unhandled key - do nothing, just redisplay current page
              // This prevents unrecognized keypresses from breaking out of the viewer
              console.clear();
              console.log('\x1b[1m\x1b[36m' + header + '\x1b[0m');
              this.displayContentPage(
                lines,
      ...
      try {
        ...
        console.log(
          `\x1b[32mFile contents saved to '${outputFilename}'.\x1b[0m`
        );
      } catch (error) {
        const err = error as Error;
        console.error(
          `\x1b[31mError saving file contents:`,
          err.message,
          '\x1b[0m'
        );
      }
  ...
  displayContentPage(
    ...
    // Show page info and only relevant navigation options
    let navInfo = `\n\x1b[33mPage ${page + 1}/${totalPages} - `;
    navInfo += `[q: return to tree]`;
    ...
    return new Promise((resolve) => {
      process.stdin.on('data', async (data) => {
        ...
        if (key === '\u001B[A') {
          ...
          // D to display content on screen
          await this.viewSelectedContent(false);
        } else if (key.toLowerCase() === 'f') {
          // F to save content to file
          await this.viewSelectedContent(true);
        } else if (key === 'd' && process.env.NODE_ENV === 'development') {
          // Toggle debug mode
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        exec('npm install prompts', (error: Error | null) => {
          if (error) {
            console.error(
              '\x1b[31mFailed to install prompts. Continuing with basic input.\x1b[0m'
            );
          } else {
